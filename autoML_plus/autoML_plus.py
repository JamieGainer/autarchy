...
def get_args():
    """ Gets arguments from command lines and returns a dictionary
        of parameter values. """
...
# default values
    ...
    param_dict = {}
    for key in [
               'quick_stop', 'seed_value', 'test_size', 'validation_size', 
               'target_column', 'verbosity', 'model', 'input_file_name',
               'trainings'
               ]:
        param_dict[key] = eval(key)

    return param_dict

def get_data(param_dict):
    """ Opens data file"""
    target_column = param_dict['target_column']
    if param_dict['input_file_name'] == 'boston':
        housing = load_boston()
    ...
    param_dict['data_shape'] = data_shape
    param_dict['target'] = target


def divide_data(param_dict):
    """ Divide data into train, test, and validation sets."""
    test_size = param_dict['test_size']
    validation_size = param_dict['validation_size']
    seed_value = param_dict['seed_value']
    ...
    for dataset in ['x_train', 'x_test', 'y_train', 'y_test', 'x_val', 'y_val']:
        param_dict[dataset] = eval(dataset)


def run_autoML_plus(param_dict):

    model = param_dict['model']
    quick_stop = param_dict['quick_stop']
    trainings = param_dict['trainings']
    ...
    x_val = param_dict['x_val']
    y_val = param_dict['y_val']
...
if __name__ == '__main__':
    param_dict = get_args()
    get_data(param_dict)
    divide_data(param_dict)
    autoML_pipeline_object = run_autoML_plus(param_dict)
